...
StatusOr<std::vector<std::string>> GraphCache::getEdges(std::string& key) {
  return cacheInternal_->getAsChain(key);
}

bool GraphCache::addAllEdges(std::string& key, std::vector<std::string>& values, uint32_t ttl) {
  if (!edgePool_) {
    LOG(ERROR) << "No edge pool exists!";
    return false;
  }
  auto status = cacheInternal_->putAsChain(key, values, edgePool_->poolName_, ttl);
  if (!status.ok()) {
    LOG(ERROR) << status;
    return false;
  }
...
void GraphCache::invalidateEdges(std::string& key) { cacheInternal_->invalidateItem(key); }
