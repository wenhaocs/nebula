...
bool GraphCache::addAllEdges(std::string& key, std::vector<std::string>& values) {
  if (!edgePool_) {
...
void GraphCache::invalidateVertex(std::string& key) { cacheInternal_->invalidateItem(key); }
